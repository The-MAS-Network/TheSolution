...
import { generateOrdinalShareLink, truncateWalletAddress } from "@/utilities";
...
const DeleteOrdinalCard = ({
  ...
  const handleDelete = async () => {
    ...
    if (!id) return appToast.Warning("Invalid ordinal id.");
    if (!ordinalCollectionId)
      return appToast.Warning("Invalid ordinal collection Id");

    const response = await deleteOrdinalAPI.mutateAsync({
      ordinalId: id,
      ordinalCollectionId,
    });
    if (response.ok) {
  ...
  return (
    ...
    >
      <p className="max-w-[15.875rem] text-center text-lg">
        ...
        {generateOrdinalShareLink(truncateWalletAddress(ordinal?.ordinalId))}{" "}

...
const pageValues = [
  ...
  "Double-check your wallet for accurate SATs sent. Enter both amounts received. Second payment may take up to 1 minute to reflect.",
  "Enter second amount",
  "Enter first amount",
  "CONFIRM",
  "RETURN TO HOME",
...
const VerifyLightningPage = (): JSX.Element => {
  ...
  const { currentTrialCount, setCurrentTrialCount, updateProfile } =
    useStore(authStore);
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    if (response.ok) {
      ...
      if (state?.purpose === "verify_account") {
        // @ts-ignore
        if (response.data) updateProfile(response.data.data);
        navigate(routes.VERIFY_LIGHTNING_SUCCESS_PAGE, {
          replace: true,
          state: {
            token: "Test Token",
